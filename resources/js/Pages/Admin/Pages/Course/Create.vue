<template>
    <layout-authen>
        <div v-if="can('Create course')" class="container mx-auto px-4 py-6">
...
export default {
    ...
    setup() {
        ...
        const submitForm = async () => {
            try {
                await axios.post('http://localhost:8000/api/courses', form.value);
                success.value = 'Course created successfully.';
                error.value = null;
                localStorage.setItem('course_created', '1');
                window.location.href = '/admin/courses';
            } catch (err) {
                error.value = err.response?.data?.message || 'Failed to create course. Please try again.';
                success.value = null;
