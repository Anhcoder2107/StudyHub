<template>
    <layout-authen>
        <div v-if="showToast && can('Create User')" id="toast-success"
        ...
        <div v-if="can('Browse User')" class="container mx-auto px-4 py-6">
            ...
            <div v-if="loading" class="text-center py-6">
                <span class="text-blueGray-600 text-lg">Loading users...</span>
            </div>
            ...
            <card-table v-else :color="tableColor" :title="tableTitle" :name-table="tableHeaders"
                :data-table="userData" :show-actions="true" :actions="(row, index) => [
                    { label: 'Edit', action: () => handleEdit(row) },
                    { label: 'Delete', action: () => handleDelete(row) },
                    { label: 'Edit Role', action: () => handleEditRole(row.id) },
                ]" />
        </div>
        <div v-if="pagination && pagination.last_page > 1 && can('Browse user')" class="flex justify-center mt-6 space-x-2">
...
import userIcon from "../../../../../css/assets/admin/img/vue.png";
...
export default {
    ...
    setup() {
        ...
        const tableTitle = ref('User Management');
        const tableHeaders = ['Name', 'Id',  'Email', 'Google ID', 'Facebook ID', 'Verify_email_at', 'Create_at', 'Update_at',  'Status'];
        const users = ref([]);
        ...
        onMounted(() => {
            fetchUsers();
            if (localStorage.getItem('user_created') === '1') {
